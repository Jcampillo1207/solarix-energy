...
export const Sec3 = ({
  ...
  const renderIcon = (icon: string | React.ReactElement) => {
    if (typeof icon === "string") {
      return <Icon name={icon} className="size-8 group-hover:text-primary" />;
    } else {
      return icon;
    }
  };
    ...
    <UniversalSection className="w-full h-fit items-start justify-start flex flex-col gap-y-10 lg:gap-y-14 xl:gap-y-24">
      ...
      <div className="w-full h-fit items-start justify-start grid grid-cols-1 md:grid-cols-2 xl:grid-cols-2 gap-3 md:gap-5 lg:gap-y-7">
        {items.map((item, index) => (
          ...
          >
            <span className="w-full h-fit items-center justify-center flex flex-col gap-y-3 text-foreground">
              <div className="bg-background flex w-full aspect-video items-center justify-center h-auto rounded-lg mb-5 border border-muted-foreground/10">
                <CardSpotlight>{renderIcon(item.icon)}</CardSpotlight>
              </div>
              <div className="text-lg text-center md:text-xl xl:text-3xl text-pretty antialiased text-foreground font-bold tracking-wide">
                {item.title}
              </div>
            </span>
            {text && (
              <div className="text-sm md:text-sm xl:text-base text-center text-muted-foreground antialiased tracking-wider text-pretty">
                {item.text}
              </div>
            )}
          </span>
